...
class OtherlinkProcess(APIView):
    def get(self, request):
        ...
        user = db_filter(User, id= user_id).first()
        session_user = db_filter(User, email= request.session['email']).first()
        if user == None:
        ...
        feed_number = db_filter(FeedLike, like_user_email=session_user.email).values_list('feed_number', flat=True) # 좋아요 버튼 정보 가져오기
