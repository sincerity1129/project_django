...
class ContentProcess(APIView):
    def get(self, request):
        ...
        user = db_filter(User, email= request.session['email']).first()
        ...
        # 왼쪽 피드 관련된 정보
        content = db_filter(Contents, nickname=user.nickname).order_by('-id') # 피드 정보 가져오기
        feed_number = db_filter(FeedLike, like_user_email=user.email).values_list('feed_number', flat=True) # 좋아요 버튼 정보 가져오기
        comment = all_db_search(Comment) # 댓글 정보 가져오기
        # 오른쪽 팔로우 관련된 정보
        exclude_user_all = db_exclude(User, email= request.session['email']) # 본인을 제외한 user 정보 가져와서 오른쪽 팔로우 영역 처리
        follow = db_filter(Follow, own_user=request.session['email']).values_list('following', flat=True) # 내가 팔로우 한 사람 목록 가져오기
        follower = db_filter(Follow, following=request.session['email']).count() # 나를 팔로우한 사람 수
    ...
    def put(self, request):
        ...
        db_update(Contents, feed_id, content_text=update_feed_content)
...
class CommentProcess(APIView):
    def post(self, request):
        ...
        user = db_filter(User, email=request.session['email']).first()
        db_create(Comment, comment_user_nickname=user.nickname, feed_number=comment_feed_number, comment_text=comment_text)
        return JsonResponse(comment["success"])
    
    def delete(self, request):
        comment_feed_id = request.data.get('comment_feed_id')
        db_delete(Comment, id=comment_feed_id)
...
def update_image_upload(request):
    if request.method == 'POST':
        ...
        content = db_filter(Contents, id=image_id).first()
    ...
    if request.method == 'POST':
        ...
        if password_update_controller=="수정하기":
            ...
            if check_password(profile_now_password, user.password):
                ...
                if profile_image_update_image_file==None:
                    db_update(User, user_email, password=make_password(profile_update_password))
