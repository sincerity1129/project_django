...
def like_click(request):
    if request.method == 'POST':
        ...
        if db_filter(FeedLike, feed_number=feed_number, like_user_email=request.session['email']).exists():
            db_delete(FeedLike, feed_number=feed_number)            
            db_update(Contents, id_query, heart_count=F('heart_count')-1)
            return JsonResponse(feed_like["cancle"])
        else:
            db_create(FeedLike, feed_number=feed_number, like_user_email=request.session['email'])
            db_update(Contents, id_query, heart_count=F('heart_count')+1)
...
def follow_add(request):
    if request.method == 'POST':
        ...
        following_user = db_filter(User, id=user_id).first()
        email_query = json_merge(email=following_user.email)

        if db_filter(Follow, own_user=request.session['email'], following=following_user.email).exists():
            db_delete(Follow, own_user=request.session['email'], following=following_user.email)
            db_update(User, email_query, follow_number=F('follow_number')-1)
            follow_info=json_merge(follow=follow["cancle"], nickname=following_user.nickname)
            return JsonResponse(follow_info)
        
        else:
            db_create(Follow, own_user=request.session['email'], following=following_user.email)
            db_update(User, email_query, follow_number=F('follow_number')+1)
